...
\begin{itemize}
\item \textbf{型号}：Tekbox TBCP2-1000 商用宽带电流探头
\item \textbf{频段}：100 kHz – 1 GHz，平坦 \(\pm 2\) dB，转移阻抗 \(Z_T \approx 5\,\Omega\)（等效插损 \(-20\) dB）
\item \textbf{一次侧最大通过电流}：30 A AC
\item \textbf{穿芯方式}：\textbf{L + N 同向穿芯}，PE 独立走线在 CT 之外。磁通 \(\Phi_\mathrm{CT} \propto I_L + I_N = 2 I_\mathrm{CM}\)，差模电流完全抵消，共模电流加倍叠加。DM 抑制经验估计 30–40 dB。
\item \textbf{工装}：Week 1 制作 2 芯延长线工装，机械保证 L+N 几何对齐，\(\delta_\mathrm{offset} \leq 0.5\,\mathrm{mm}\)
\end{itemize}

\paragraph{辅路（HV 电容）}

\begin{itemize}
\item \textbf{元件}：Murata GA355DR7GF472KY02（4.7 pF / 1 kV NP0）\(\times 2\) 串联（双重失效保护）
\item \textbf{接入点}：DUT 电源线 L 线对地，经 LC HPF（\(f_c = 200\) MHz）+ 50 \(\Omega\) 端接 \(\to\) SMA \(\to\) LDSDR RX2
\item \textbf{频段}：200 MHz – 2 GHz，补足 CT 高频段
\item \textbf{安全}：即使一颗电容击穿短路，另一颗仍保持 \(> 500\) V 工作电压余量
\end{itemize}
...
\item PC 端按 EM Eye 论文 Eq.3 做相干合成，SNR 提升估算 +3 dB
\end{itemize}

与论文原方案的差异化：论文用单 USRP \textbf{时分}采样不同频段；本方案用 LDSDR 2 RX \textbf{同时}采样，这是 v1 baseline 就具备的硬件优势。

\subsection{保护与工频隔离链}
\label{sec:ch2-protect}

完整保护链结构（占位图：\texttt{figures/proposal\_protection\_chain.png}）：

\begin{lstlisting}[basicstyle=\ttfamily\footnotesize]
[Tekbox CT 二次侧 BNC 输出 50 Ohm]
        |  RG-316 短同轴 (< 20 cm)
        v
...
[DC 隔直: Murata GRM18 NP0 1 nF / 100 V]   <-- 阻 DC, 通 RF
   50 Hz 容抗 3.18 MOhm / 30 MHz 容抗 5.3 Ohm / 1 GHz 容抗 0.16 Ohm
        |
        v
[LC HPF 4 阶 Butterworth 50 Ohm, fc = 30 MHz]
   C1=150 pF NP0, L2=150 nH Coilcraft 0603HP (Q>50@100MHz),
   C3=56 pF NP0,  L4=330 nH
   50 Hz 衰减 > 100 dB (实测目标), 通带 30 MHz - 3 GHz 损耗 < 1 dB
        |
        v
[LNA 输入端 -> ERA-4SM+ x 2 级联板]
\end{lstlisting}

\textbf{Phase 2（Route B 演进）新增市电入口保护}：GDT（Bourns 2026-23-SM）\(\to\) MOV（Littelfuse V275LA20A）\(\to\) 共模扼流圈（Würth 744232222），用于内含 AC-DC 的插头形态。Phase 1 USB 供电时这些元件不需要。

\paragraph{关键工程规则}

\begin{enumerate}
\item \textbf{信号路径绝对不加共模扼流圈}：本设备的"目标信号"就是共模电流本体，信号侧加共模扼流圈等于扼掉信号。
\item \textbf{TVS 选低 C 型号}：普通 TVS（SMAJ12A）寄生电容 \(\sim 200\) pF，在 1 GHz 容抗仅 0.8 \(\Omega\) 几乎短路；必须用 \(C < 1\) pF 的 RF 专用 TVS。
\item \textbf{隔直电容选 NP0/C0G 介质}：温度系数 \(\pm 30\) ppm/°C，Q 值高，无压电效应；X7R 介质机械振动会产生附加噪声，在 \(\mu\)V 级信号上不可接受。
\end{enumerate}
...
\begin{enumerate}
\item \textbf{W1}：Tekbox CT 到货 \(\to\) 实验室 VNA + 频谱仪验证 S21 频响，扫频 100 kHz – 1.5 GHz，要求 100 kHz – 1 GHz 平坦 \(\pm 3\) dB
\item \textbf{W1}：候选人 BYO ERA-4SM+ \(\times 2\) LNA 与 LDSDR 集成，验证整链注入校准（信号发生器 \(-80\) dBm \(\to\) LDSDR RSSI \(-20\) dBm，匹配 +60 dB 增益）
\item \textbf{W2}：RPi 4B + Camera V1.3 通电（论文同款 DUT），Tekbox 夹电源线，频谱仪扫 50 MHz – 1 GHz，搜索 30 Hz 周期性载波，\textbf{目标频点 204 / 255 MHz}
\item \textbf{W2–W3}：LDSDR 8 MSPS IQ 采集，Python 端幅度解调 + 30 Hz 自相关 \(\to\) Tf/Tr 估计 \(\to\) 像素重排
\item \textbf{W3}：扩展到 3 个 COTS 设备（Wyze Cam Pan 2 / Xiaomi Dafang / 360 行车记录仪），重复 (3)(4)
\end{enumerate}
...
候选人在加入项目之前\textbf{已完成} ERA-4SM+ \(\times 2\) 级联 LNA 模块的设计、加工与 VNA 实测验证（详见附录 B）。本提案的模拟前端直接复用该模块，不需要额外开发。
...
\textbf{论文对标}：EM Eye 论文 Appendix H 使用 Foresight FST-RFAMP06（40 dB 单级），候选人 ERA-4SM+ \(\times 2 = 28\) dB，\textbf{差 12 dB}。这 12 dB 由 LDSDR AD9363 内部 LNA + IF VGA（最高 30 dB）弥补，且 AD9363 内部级联点在 LNA 之后，\textbf{对系统 NF 影响 \(< 0.1\) dB}（Friis 公式）。
...
\subsection{可切换 BPF 组（可选，Phase 1）}
\label{sec:ch3-bpf}
...
\begin{table}[H]
\centering
\caption{Stage 2 BPF 配置}
\label{tab:bpf-stage2}
\small
\begin{tabularx}{\textwidth}{cXXXX}
\toprule
\rowcolor{tableheader}
\textbf{段} & \textbf{中心/带宽 (MHz)} & \textbf{SAW 型号} & \textbf{IL/抑制 (dB)} & \textbf{覆盖目标} \\
\midrule
...
\textbf{RF 开关}：Peregrine PE42423 SP4T，DC – 6 GHz，IL \(< 0.7\) dB，隔离 \(> 50\) dB，切换 \(< 1\,\mu\)s。候选人在 XCZU3EG PL LPR CNN 项目已有 PE42423 PCB 布板与 SPI 控制经验。\textbf{Stage 2 BOM}：¥ 350（3 SAW + 1 开关 + 偏置），作为应急储备。
...
\begin{itemize}
\item 模拟前端板装铜罩（顶 + 底），罩接 L2 模拟地，\textbf{单点接地}到 LDSDR SOM 入口
\item \textbf{LNA 单独子腔屏蔽}，与滤波/耦合区物理隔离，避免反馈振荡
\item 模拟域与数字域接地通过单个 0 \(\Omega\) 跳线（Star Ground）
\item SMA 接头 PCB 直焊，via stitching 间距 \(\leq 1/10\) 波长（@ 1 GHz 即 30 mm）
\end{itemize}

\paragraph{电源去耦} LNA 偏置 5 V \(\to\) TI TPS7A47（4 nV/\(\sqrt{\mathrm{Hz}}\) LDO）\(\to\) 0.1 \(\mu\)F + 10 \(\mu\)F MLCC + 100 \(\mu\)F 钽近端布置；LDSDR USB 5 V \(\to\) TPS54320（5 V \(\to\) 1.8 V 数字）+ 独立 LDO 给 AD9363 RF 电源。

\paragraph{预期 EMI 屏蔽效能} 铝合金外壳 + 铜罩 = 40 – 60 dB @ 1 GHz（覆盖 WiFi 2.4G / FM / GSM 900 等外部干扰源）。
...
候选人 BYO 的 \textbf{LDSDR 7010 rev2.1} 取代原计划的 ADALM-Pluto。
...
\textbf{关键差异化点}：候选人在 LDSDR 同款 Zynq-7010 上已实现 OFDM + LDPC 完整 HDL 收发机，BER = 0 板级验证（详见附录 B）。这意味着：

\begin{itemize}
\item LDSDR Buildroot rootfs（Linux 5.15）、iio\_attr 用户态控制、Vivado 工程结构、AD9363 SPI 配置流程\textbf{全部已通}
\item Phase 2 \texttt{emeye\_accel} RTL 开发\textbf{直接复用} OFDM 项目的 AXI-Stream / AXI-Lite / IRQ 框架
\item XCZU3EG HLS LPR CNN 项目（87.94 \% / 675 ms 端到端）同样适用于 LDSDR 板上 pix2pix 加速备份方案
\end{itemize}
...
\paragraph{开机扫描（Phase 1 完成后）}

\begin{enumerate}
\item \textbf{快扫}：50 MHz – 1 GHz 以 1 MHz 步长扫频，每点 100 ms，记录功率谱
\item \textbf{粗候选}：挑出 top-5 强谱线（信号 \(>\) 噪底 + 15 dB）
\item \textbf{细识别}：对每个候选频点做 IQ 采集 1 秒，FFT + 30 Hz 周期性自相关识别真实 EM Eye 泄漏（其他干扰如 FM 广播无 30 Hz 周期性）
\item \textbf{锁定 + 漂移补偿}：选 SNR 最高的频点持续锁定；LDSDR AD9363 TCXO \(\pm 25\) ppm，叠加 PC 端软件 PLL 补偿 \(\pm 50\) ppm 范围
\end{enumerate}

频点漂移来源：DUT 内部 PLL 与攻击设备 LO 频率差，典型 \(< 50\) ppm，在 1 GHz 处即 \(\pm 50\) kHz，远小于 8 MSPS 带宽，软件补偿无压力。
...
\textbf{选定路径}：\textbf{v1 千兆 Ethernet 原始 IQ 直传}（双 RX 384 Mbps，占用千兆 \(\sim 48\,\%\)），Phase 2 在板上做 \texttt{emeye\_accel} 加速。v1 让算法快速迭代（Python 改一行立即测试），v2 在算法稳定后把热路径下沉到 FPGA，数据率降低 12 倍（384 \(\to\) 32 Mbps），为 Phase 3 无线版本铺路。
...
原计划在 LDSDR 后接 Raspberry Pi CM4 8 GB Lite 作为主控。Day 1 重新评估后\textbf{取消该决策}，直接用 LDSDR 板内 Zynq-7010 PS。
...
\textbf{LDSDR PS 端}（Buildroot Linux 5.15，候选人 BYO）：\texttt{iio\_attr} 配置 AD9361 LO/增益/采样率/滤波器；libiio-dame 服务端通过千兆 socket 把 IQ 流推送到 PC；Python 端 GNURadio + pyadi-iio 实时采集。

\textbf{PC 端}（笔记本）：接收双 RX IQ 流（对齐时戳）\(\to\) 频点跟踪 + AGC 反馈 \(\to\) 幅度解调 + 30 Hz 自相关 \(\to\) Tf/Tr 估计 \(\to\) 像素重排 + pix2pix GAN 精修（GPU 加速）。

\subsection{Sidebar — 板上 FPGA 加速路线图（Phase 2）}
\label{sec:ch4-accel}
...
\textbf{候选人能力背书}：\texttt{emeye\_accel} RTL 候选人已在 Pre-Phase 0 完成首版 Vivado 工程（详见附录 B），仿真通过；LDPC 项目 BER = 0 板级验证证明 PS-PL 协同链路稳定；XCZU3EG HLS LPR 项目 675 ms 端到端记录证明可在 FPGA 上跑接近 pix2pix 复杂度的网络。
...
\paragraph{Phase 0 — 可行性验证（W1–W3，3 周）} Tekbox CT 标定 + RPi V1 复现 + COTS 设备扫描 + Go/No-Go 决策门；交付 Phase 0 实验报告 + Phase 1 设计修正建议。

\paragraph{Phase 1 — 工程原型（W4–W15，12 周）}

\begin{itemize}
\item W4–W7：模拟前端板设计 + PCB 打样 v1 + LNA / 滤波链联调
\item W8–W11：LDSDR Buildroot rootfs + emeye 算法 Python + 多频段双 RX 融合
\item W12–W14：PCB 打样 v2 + 3D 外壳 + 整机集成 + 多 DUT 验证
\item W15：Phase 1 评审，出 v1 完整工程原型
\end{itemize}

\paragraph{Phase 2 — 集成 + 验证（W16–W24，9 周）}

\begin{itemize}
\item W16–W19：\texttt{emeye\_accel} RTL 板上集成 + PS-PL 联调（OFDM 经验直接迁移）
\item W20–W22：6 层集成 PCB + CNC 外壳 + 自研小型化 CT 验证
\item W23–W24：多设备 + 多场景验证 + 终版报告
\end{itemize}

\paragraph{Phase 3（可选，W25+）} Route B 插头形态 + EMC Class B 预测试 + IEC 61010 安规。\textbf{不在本提案 6 个月预算内}。
...
\textbf{预算合理性}：(1) BYO 资产省去 SDR + LNA 大头（否则 \(\sim\) ¥ 4{,}500 + 自研 LNA 工时）；(2) Phase 0 Go/No-Go 限制沉没成本（即使 No-Go 投入 \(<\) ¥ 10{,}000 即可终止）；(3) Phase 2 学术探索与 Phase 1 主路径解耦；(4) LISN/频谱仪/VNA 假设实验室公共资源借用。

\subsection{风险驱动的预算保留}
\label{sec:ch6-reserves}

\begin{table}[H]
\centering
\caption{Phase 0 后可能触发的应急预算}
\label{tab:reserve-budget}
\small
\begin{tabularx}{\textwidth}{XXrX}
\toprule
\rowcolor{tableheader}
\textbf{风险} & \textbf{触发条件} & \textbf{应急 (¥)} & \textbf{应对} \\
\midrule
...
\textbf{应急预备金合计} & & \(\sim\) \textbf{6{,}350} & 占总硬件预算 \(\sim 30\,\%\) \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{itemize}
\item \textbf{平台}：Analog Devices PlutoSDR / LDSDR（同款 \texttt{xc7z010clg225-1}）
\item \textbf{角色 / 时间}：单人独立完成 PHY + Baseband 全部 HDL 设计与板级联调 / 约 8 个月
\item \textbf{业务}：自研收发机验证 OFDM-LDPC 联合编码在低成本 SDR 上的工程可行性
\end{itemize}

\paragraph{关键技术} 完整 OFDM 链路 HDL（FFT/IFFT、循环前缀、导频插入、信道估计、均衡）；LDPC 编解码器 HDL（码字可配置，min-sum 迭代译码 + 比特节点并行）；时频同步（Schmidl-Cox 粗同步 + 导频辅助 CFO）；Pluto Zynq-7010 PL 综合通过，时序闭环 30.72 MHz；GNURadio 上位机 baseband I/Q 实时收发与 BER 统计。

\paragraph{关键结果} \textbf{板级实测 BER = 0}（SNR \(\geq 10\) dB，\(10^6\) bit 连续传输零误码）；Vivado 资源 LUT 38\% / BRAM 52\% / DSP 24\%（xc7z010）；端到端吞吐 1.5 Mbps（QPSK, 1/2 LDPC）。

\paragraph{对本提案的能力支撑} 本项目是候选人\textbf{能改 Pluto/LDSDR Zynq-7010 内部 HDL 并通过 bitstream 板级验证}的直接证据，是 Sidebar B"板上 FPGA 加速"路径的核心能力前置。详见 §A.8 映射表。

\paragraph{工具链} Vivado HLS 2020.2 / Verilog/SystemVerilog / GNURadio 3.8（私有仓库，可在面试时演示）。

\subsection{XCZU3EG PL CNN 车牌识别端到端部署}
\label{sec:appA-2}

\begin{itemize}
\item \textbf{平台}：Xilinx Zynq UltraScale+ MPSoC XCZU3EG（FZ3A 板，APU A53 \(\times 4\) + RPU R5 + PL）
\item \textbf{角色 / 时间}：单人完成 CNN 模型选型、HLS 实现、PL 部署、Petalinux 集成 / 约 6 个月
\item \textbf{业务}：边缘端 LPR demo，验证 HLS 在中等算力 PL fabric 上的可部署性
\end{itemize}

\paragraph{关键技术} LPRNet 轻量化 CNN + CTC loss，参数量约 0.5 M；Vivado HLS（卷积 line-buffer + 行流水，激活 LUT 化，定点 INT8）；PL/PS 协同（AXI4-Stream DMA 送图，中断回收结果）；多级流水线 Conv-BN-ReLU 三段融合，pipeline II=1，时序闭环 200 MHz；Petalinux 自定义 image。

\paragraph{关键结果} \textbf{字符识别准确率 87.94\%}（2000 张测试集）；\textbf{端到端延迟 675 ms}（含 PS\(\leftrightarrow\)PL DMA + PL 推理 + PS CTC 解码）；PL 资源 LUT 71\% / DSP 83\% / BRAM 64\%。

\paragraph{对本提案的能力支撑} \textbf{Vivado HLS 多级流水线设计 + RTL 级时序闭环 + PS/PL 协同}经验，可直接迁移到 \texttt{emeye\_accel} IP 设计与板上 PS Linux + PL IP 通信架构。

\paragraph{仓库} \href{https://github.com/stongry/FPGA-ZYNQ}{github.com/stongry/FPGA-ZYNQ}；工具链 Vivado HLS / Petalinux / Vitis AI 1.4。

\subsection{QSM368ZP RK3568 Ubuntu 22.04 移植与 RKNN NPU 部署}
\label{sec:appA-3}

\begin{itemize}
\item \textbf{平台}：Rockchip RK3568（4\(\times\)A55 + Mali-G52 + 0.8 TOPS NPU）/ QSM368ZP 工业模块
\item \textbf{角色 / 时间}：单人完成从 Buildroot 到 Ubuntu 22.04 的全量移植 + RKNN 推理生态 / 约 4 个月
\item \textbf{业务}：客户出货级 Buildroot 替换为标准 Ubuntu 便于二次开发，同时对接 NPU 推理生态
\end{itemize}

\paragraph{关键技术} U-Boot + Linux 5.10（Rockchip BSP）+ DTS 适配 QSM368ZP 引脚 mux；Ubuntu 22.04 server rootfs + systemd + Rockchip GPU/NPU blob；DSI 触摸控制器驱动调试；RKNN Toolkit 2.3.2 + librknnrt + ONNX\(\to\)RKNN 流水线；RetinaFace 端到端 USB camera \(\to\) RGA \(\to\) NPU \(\to\) NMS 全链路打通。

\paragraph{关键结果} \textbf{生产级嵌入式 Linux 已交付客户}；\textbf{RetinaFace 端到端 41.7 fps @ RK3568 NPU}；NPU 单帧推理 15 ms（见 §A.6）。

\paragraph{对本提案的能力支撑} Phase 2 板上 PS Linux 侧 TCP forwarder / 帧 IQ 落盘服务（设备树修改 + udev rules + systemd）+ 若需 SSIM offload 到 NPU 的 RKNN 流水线经验。工具链 Buildroot / Yocto / Rockchip BSP / RKNN Toolkit 2.3.2（私有仓库）。

\subsection{EC800M LTE Cat-1 模块语音 AI 对话集成}
\label{sec:appA-4}

\begin{itemize}
\item \textbf{平台}：Quectel EC800M（MDM9205，LTE Cat-1 + 自带 TTS/Audio）
\item \textbf{角色 / 时间}：单人完成 QuecPython 应用 + LTE 拨号 + 云端语音 AI 链路 / 约 3 个月
\item \textbf{业务}：客户"插卡即用"语音 AI 对话硬件，出货级模块
\end{itemize}

\paragraph{关键技术} QuecPython 纯 Python 业务栈；LTE Cat-1 PPP 拨号 + TCP/TLS 长连接到云端 ASR/LLM/TTS；板载 audio codec 16 kHz PCM；低延迟流式 TTS + 半双工抢答；AT 命令 OTA。

\paragraph{关键结果} \textbf{出货级语音 AI 模块批量出货}；从 SIM 注网到首字延迟约 800 ms。

\paragraph{对本提案的能力支撑} Phase 3（可选扩展）远程 telemetry / 长距离回传场景，具备 LTE 集成 + 云端长连接 + OTA 经验；AT 命令调试与 TLS 客户端对 PS 侧网络栈调试有间接帮助。\textbf{与本提案主线关系较间接，列此处体现出货级嵌入式产品交付经验}。

\paragraph{仓库} \href{https://github.com/stongry/2026yiyuan}{github.com/stongry/2026yiyuan}（\texttt{darken\_neko/ec800m-voice-chat}）。

\subsection{Smart Home Slint UI 全量移植（Flutter \texorpdfstring{$\to$}{->} Slint）}
\label{sec:appA-5}

\begin{itemize}
\item \textbf{平台}：RK3588 显控面板（LubanCat-5，4\(\times\)A76 + 4\(\times\)A55，Mali-G610）
\item \textbf{角色 / 时间}：单人完成 Flutter \(\to\) Slint 全量重写 / 约 2 个月
\item \textbf{业务}：原 Flutter 帧率不稳 + GPU compositor 重，换轻量原生 UI 释放 GPU
\end{itemize}

\paragraph{关键技术} Slint UI 框架（Rust）+ 自定义 widget 库；交叉编译 \texttt{aarch64-unknown-linux-gnu}；Wayland + EGL 后端规避 X11 合成；Rust 异步（tokio）+ Slint 属性桥接；layout 抖动消除 + GPU 状态切换优化。

\paragraph{关键结果} \textbf{62.97 fps，达到面板硬件 60 Hz vsync 上限}（始终不掉帧）；二进制体积比原 Flutter 下降约 70\%。

\paragraph{对本提案的能力支撑} Phase 2/3 若需重建 EM Eye 实时可视化上位机（帧显示 + SSIM 曲线 + 频谱图），具备 Rust + Slint 高性能 UI 工程能力；GPU/Wayland 合成栈理解可迁移到 Qt/Dear ImGui。工具链 Rust 1.75 / Slint 1.4 / Wayland。

\subsection{RetinaFace NPU 性能 Benchmark（RK3568 vs RK3588）}
\label{sec:appA-6}

\begin{itemize}
\item \textbf{平台}：对比 RK3568（QSM368ZP）与 RK3588（LubanCat-5）
\item \textbf{角色 / 时间}：单人设计 benchmark 方法学并执行 A/B 实测 / 约 2 周
\item \textbf{业务}：内部选型决策，严格量化两代 Rockchip NPU 端到端性能差距
\end{itemize}

\paragraph{关键技术} 严格 A/B 控制（同 ONNX + 同 RKNN 转换 + 同测试集 + 同推理代码）；测量分离（纯 NPU kernel \texttt{rknn\_run} / PS\(\leftrightarrow\)NPU 数据搬运 / 前后处理 CPU）；每平台 1000 帧重复，中位数 + p95 + p99；CPU performance governor 固定 + 单线程绑定。

\paragraph{关键结果} \textbf{NPU 单帧} RK3568 = 15 ms，RK3588 = 7.75 ms（\(\sim\) 2\(\times\) 加速）；\textbf{端到端 fps} RK3568 = 20.7，RK3588 = 41.7。结论已写入内部选型报告。

\paragraph{对本提案的能力支撑} Phase 0/1 大量 A/B 实测（基线 vs 改进）严谨方法学，完整实验设计 + 数据采集 + 统计报告能力。

\subsection{本提案 Pre-Phase 0 已完成工作}
\label{sec:appA-7}

\begin{itemize}
\item \textbf{平台}：本提案目标平台（LDSDR + 自研耦合电路 + PC 仿真）
\item \textbf{角色 / 时间}：候选人自费在面试前完成的预研工作 / 约 6 周
\end{itemize}

\paragraph{关键交付件}

\begin{itemize}
\item \textbf{5 个 Python 端到端仿真}：覆盖 EM Eye 信号建模、HDMI TMDS bit-level 仿真、TCP demo，\textbf{帧级 SSIM = 0.99}
\item \textbf{4 个 RTL testbench 全部 PASS}：Verilator / Vivado Simulator 双重验证
\item \textbf{LDSDR 真目标 bitstream 已生成 + 烧录 + RF 实测}：在 Zynq-7010 上启动正常，采集真实 RF 数据回放
\end{itemize}

详见附录 B。\textbf{§A.7 不是"过去的项目"，而是候选人对本提案诚意与执行力的直接证据}：在尚未拿到 RA offer 前已自费 6 周完成 Pre-Phase 0，入职即可直接进入 Phase 1。
...
\textbf{经验等级说明}：\textbf{熟练} = 作为主负责人独立完成过端到端项目并交付板级或客户出货结果；\textbf{有经验} = 参与过相关项目部分模块，理解关键工程难点；\textbf{入门} = 理解原理可指导下完成，无独立交付经验。
...
\begin{enumerate}
\item \textbf{2 份端到端 Python 仿真}（EM Eye 复现 SSIM 0.98 + HDMI TEMPEST 扩展 SSIM 0.9907）+ \textbf{1 份 LDSDR \(\to\) 主机 TCP demo}
\item \textbf{4 个可综合 Verilog 模块} + \textbf{4 个 testbench 全部 PASS}（Icarus Verilog，无 Vivado license 依赖）
\item \textbf{完整 Vivado 流程}到 \texttt{write\_bitstream}，目标真器件 \textbf{XC7Z010-CLG400-2}（LDSDR rev2.1），产物 \texttt{.bit} MD5 已固定
\item \textbf{真板烧录 + AD9361 RF 实测}：千兆 GbE 在线，FPGA Manager \texttt{operating}，RX\_LO 配到 \textbf{204 MHz}，RSSI \textbf{93.75 dB}，采集 16 KB 真实 IQ，dmesg 零错误
\item \textbf{Phase 2 提前规划文档}（AXI-Stream tap 方案 + PS 用户态 TCP forwarder 骨架共 1096 行）
\end{enumerate}

工程产物总计 \textbf{约 6{,}000 行}（Python + Verilog + C + Markdown），全部在本地 \texttt{/home/ysara/work/powerline-emeye-proposal/} 下。
...
文件 \texttt{simulation/hdmi\_simulation.py}（423 行）证明：\textbf{论文方法不是 EM Eye 专用工具，而是通用 EM 侧信道重建平台}。
...
文件 \texttt{simulation/network\_demo.py}（405 行）：Python 模拟 FPGA 加速器输出 + TCP 传输 + 主机重建，\textbf{主机侧代码全部预先验证完毕}，Phase 2 真硬件 bring-up 时只需把 Python mock 换成真 socket。
...
\texttt{tb\_emeye\_accel} 在仿真期发现并修复了一个\textbf{真实 RTL bug}：顶层 round-robin 输出无缓冲导致样本丢失。修复方案是加 pending buffer + 优先级回退。这一条单独足以证明候选人不是"跑通即提交"。
...
\textbf{目标器件：\texttt{xc7z010clg400-2}（LDSDR rev2.1 真硬件）}。不是 PlutoSDR 衍生 z020，候选人专门核对过 LDSDR 原理图。
...
\textbf{关键结论}：Phase 3 多频段融合需要至多再加 3-4 路并行处理通道，\textbf{当前 86\%+ 资源余量完全 cover 得住}。这是"多频段融合可行"claim 的硬证据。
...
跨时钟域 WNS 负值不是候选人 RTL 引入的，是 LDSDR\_2TR 模板把 \texttt{rx\_clk} 直接打到 AXI 寄存器读侧的历史遗留。Phase 1 W1 计划的缓解措施二选一：
...
候选人\textbf{没有停在"烧录成功"这一步}，而是继续把 AD9361 配到 EM Eye 论文方法关心的 byte-clock 谐波频段并真实接收 RF：
...
非零，典型空气接收 baseline 噪声 + 信号（signed 12-bit little-endian：\texttt{0xfff0} \(\to -16\)，\texttt{0xffd5} \(\to -43\)，等等）。\textbf{这意味着 RX path 是真的导通的，不是空采}。工具链全部用 \texttt{iio\_attr} + sysfs 直接读写，无需 libiio C API 介入。

\subsubsection{这一步对提案的意义}

把这一步在提案阶段做完，意味着 \textbf{Phase 1 W1 的硬件 bring-up 风险被前置消除}：LDSDR 进 lab 当天就可直接进入 Phase 1 Day 3（emeye\_accel IQ tap 接线），省 1-2 周。

\subsection{Phase 2 准备文档（提前完成）}
\label{sec:appB-5}

为让 Phase 2 不在文档阶段卡壳，候选人已写完两份 451 + 251 + 394 = \textbf{1{,}096 行}工程预案。

\subsubsection{Phase 2 IQ Tap 方案}

\texttt{fpga\_accel/doc/phase2\_iq\_tap.md}（451 行）：

\begin{itemize}
\item BD（Block Design）中把 \texttt{axi\_ad9361} ADC AXI-Stream tap 到 \texttt{emeye\_accel\_top} 的两套方案对比：
  \begin{itemize}
  \item \textbf{方案 A}（顶层手动接）：优点直观，缺点改顶层 verilog 工程文件混乱
  \item \textbf{方案 B}（BD 内 IP block 包装）：优点工程整洁，缺点要写 IP packaging。候选人推荐方案 B
  \end{itemize}
\item \textbf{5 项待验证清单}（Day 1-2 必做）：时钟域、stream 位宽、tready 反压、空 idle 处理、reset 序列
\item Phase 2 第一周 \textbf{Day by Day 任务拆解}（D1: tap \(\to\) D2: 单通 \(\to\) D3: 双通 \(\to\) D4: 同步触发 \(\to\) D5: 联调）
\end{itemize}

\subsubsection{Phase 2 PS 用户态 TCP Forwarder}

\texttt{fpga\_accel/doc/phase2\_tcp\_forwarder.md}（251 行）+ \texttt{phase2\_tcp\_forwarder.c}（394 行 C 骨架）：

\begin{itemize}
\item \textbf{AXI-DMA S2MM 寄存器}全部标注 Xilinx \textbf{PG021} 出处（MM2S\_DMACR / SA / LENGTH / S2MM\_DMASR 等）
\item 关键设计点：\textbf{UIO + mmap} 用户态访问 AXI 寄存器（不写 kernel driver）；\textbf{ping-pong buffer}（2\(\times\)4MB）避免 DMA 等 socket；\textbf{TCP\_NODELAY + SO\_SNDBUF=4MB} 优化吞吐；跟 \texttt{simulation/network\_demo.py} 客户端协议兼容（同 32-byte 帧头 + little-endian payload）
\end{itemize}
...
static uint8_t *buf[2];   // 2 x 4MB, bus-aligned
static int active = 0;
...
Phase 2 W2 主机侧只要把 \texttt{simulation/network\_demo.py} 的 mock socket 接到这个 forwarder，管线就闭环。
...
\textbf{本提案不是规划书，是已经动工的工程报告}。Phase 0 关键工程风险（论文方法理解、RTL 可综合性、目标器件资源容量、真板可控性、RF 链路在论文频点的可用性、Phase 2 主机管线协议）在提案提交之前已被逐一前置消除。

候选人请求 RA 录用后，\textbf{Phase 1 W1 第 1 天可以直接进入 \texttt{emeye\_accel} 与 \texttt{axi\_ad9361} IQ tap 的真板联调}，而不是再花 1-2 周做 bring-up。
...
\paragraph{Go/No-Go \#1 触发条件} Phase 0 W2 末若 LISN 实测电源线在 100 MHz–1 GHz 段耦合衰减 \(> 65\) dB，则：

\begin{itemize}
\item 路线 A：申请实验室更高增益 LNA（40–60 dB，单级 NF \(< 2\) dB）
\item 路线 B：重新评估 EM Eye 场景，先聚焦近场 + 短电源线段（\(< 2\) m）
\end{itemize}

\paragraph{Go/No-Go \#2 触发条件} Phase 0 W3 末若 RPi V1 在隔离变压器后端电源线上 SNR \(< 15\) dB，则：

\begin{itemize}
\item 路线 A：换 DUT（选 Wyze Cam Pan / Dafang 1080P 等 Table II 泄漏更强的）
\item 路线 B：增加被动滤波器组（BPF SAW + 模拟下变频）前置 LDSDR
\end{itemize}
