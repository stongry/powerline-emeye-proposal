...
\textbf{关键差异化点}：候选人 LDSDR 同款 Zynq-7010 上已实现 OFDM + LDPC 完整 HDL 收发机，BER = 0 \statusok（附录 B）。直接复用项：

\begin{itemize}
\item LDSDR Buildroot rootfs（Linux 5.15）、iio\_attr 用户态控制、Vivado 工程结构、AD9363 SPI 配置流程 \statusok
\item Phase 2 \texttt{emeye\_accel} RTL 复用 OFDM 项目的 AXI-Stream / AXI-Lite / IRQ 框架
\item XCZU3EG HLS LPR CNN 项目（87.94\% / 675 ms 端到端）适用于 LDSDR 板上 pix2pix 加速备份方案
\end{itemize}
...
\paragraph{对本提案的能力支撑} Pluto/LDSDR Zynq-7010 内部 HDL 修改 + bitstream 板级验证经验 \(\to\) 直接支撑"板上 FPGA 加速"路径。详见 §A.8 映射表。工具链 Vivado HLS 2020.2 / Verilog / GNURadio 3.8（私有仓库）。
...
\paragraph{对本提案的能力支撑} PS/PL 协同 \(\to\) 适用 \texttt{emeye\_accel} PL IP + PS Linux 通信。仓库 \href{https://github.com/stongry/FPGA-ZYNQ}{github.com/stongry/FPGA-ZYNQ}；工具链 Vivado HLS / Petalinux / Vitis AI 1.4。
...
\paragraph{对本提案的能力支撑} 出货级嵌入式产品交付经验 + LTE 集成 + 云端长连接 + OTA。仓库 \href{https://github.com/stongry/2026yiyuan}{github.com/stongry/2026yiyuan}（\texttt{darken\_neko/ec800m-voice-chat}）。
...
\paragraph{对本提案的能力支撑} 上位机可视化（帧显示 + SSIM 曲线 + 频谱图）Rust + Slint 高性能 UI 能力。工具链 Rust 1.75 / Slint 1.4 / Wayland。
...
\paragraph{对本提案的能力支撑} Phase 0/1 A/B 实测严谨方法学 \(\to\) 完整实验设计 + 数据采集 + 统计报告。
