...
## 三、Phase 0 设备需求清单 🟡(被雇佣后 Week 1)

**重要说明**:本清单是**项目启动需求**,**不是候选人个人采购计划**。所有设备应由:
1. **实验室公共资源借用**(首选,大部分通用 RF 测试设备应已有)
2. **项目启动经费采购**(实验室无的部分)
3. **候选人 BYO 资产**(已有的:LDSDR + ERA-4SM+ × 2 LNA + 工具链)

候选人在提案阶段**不预先购买任何设备**。下表所有金额是 Phase 0 总需求成本估算,供项目预算评估用。

**预算上限**:¥10,000

| 项 | 型号 / 规格 | 用途 | 优先来源 | 估算成本(若需采购)| 小计 |
|---|---|---|---|---|---|
| **宽带电流探头** | Tekbox TBCP2-1000(100kHz-1GHz)或同等 | 电源线传导信号采集 | 🔵 实验室借用首选 | ¥4,500 | 4,500 |
| **频谱仪** | 9 kHz – 3 GHz | 信号链调试 | 🔵 实验室通用 | (借用) | 0 |
| **矢量网络分析仪** | DC – 3 GHz | 前端板 + CT S 参数验证 | 🔵 实验室通用 | (借用) | 0 |
| Raspberry Pi 4B 套件 | 4GB RAM + 32GB SD + 电源 | 受害设备(论文同款) | 🟡 需采购 | ¥500 × 2 | 1,000 |
| RPi Camera Module v1.3 | OV5647 sensor | EM Eye 论文同款 | 🟡 需采购 | ¥80 × 2 | 160 |
| SMA 同轴线和接头 | 3 米 RG-316,公母转接 | 信号链 | 🟡 需采购 | ¥30 × 10 | 300 |
| 隔离变压器 | 1:1, 220V/220V, 1kVA | 安全调试 | 🔵 实验室借用,或采购 | ¥1,200 | 1,200 |
| 目标 COTS 摄像头 ×4 | Wyze Cam Pan 2 / Xiaomi Dafang 等 | 多设备验证 | 🟡 需采购 | ¥200-500/台 | 1,200 |
| 杂项(电阻/电容/小工具) | 调试用 | | 🟡 需采购 | ¥500 | 500 |
| Fair-Rite 磁芯样品集(可选) | Phase 2 自研 CT 探索 | 学术延伸方向 | 🟡 Phase 2 需采购 | ¥300 | 0(Phase 0 不需要) |
| **Phase 0 总需求** | | | | | **~¥8,860** |
| **实际采购(扣除实验室可借用)** | | | | | **~¥3,160 – ¥8,860** |

**关键判读**:
- 如果实验室有 Tekbox 类宽带 CT + 频谱仪 + VNA + 隔离变压器 → 实际采购仅 ¥3,160(消费品)
- 如果实验室什么都没有 → 全套采购 ¥8,860(主要是 Tekbox 大头)
- **询问 PhD 是关键**(见 phd_questions.md):他们的回答决定我们采购清单的真实金额
