...
### 总预算汇总(方案 A:LDSDR + Ethernet,USB 供电)

| 类别 | 金额(CNY) | 备注 |
|---|---|---|
| 🟢 候选人 Bring-Your-Own 资产 | (估值 ¥10-15k,不计预算) | LDSDR + ERA-4SM+×2 + Vivado/HLS 工具链 |
| 🟡 Phase 0 采购 | 5,010 | 不买商用探头,自研最小 CT |
| 🟠 Phase 1 BOM(单台原型) | 820 – 1,170 | 砍掉 CM4 + 数字载板 + Stage 2 BPF |
| 🟠 Phase 1 NRE(PCB/外壳/耗材) | 1,800 | PCB 打样 1500 + 外壳 300 |
| 🔴 Phase 2 采购(集成 + 备份) | 4,000 | LDSDR 备机 + Phase 2 PCB 二代 |
| **总预算**(方案 A) | **~¥11,600** | 不含 RA 工资 |
| **加 RA 工资**(6 个月,按 2k/月津贴) | +12,000 | |
| **项目总成本** | **~¥23,600** | |

**对比演进**:
- 初版方案(AC 供电 + CM4 + 全保护链):¥30,000
- USB 供电改进后:¥29,000
- **方案 A(LDSDR + Ethernet)**:**¥23,600**(累计省 ¥6,400)

**核心进步**:
- ✅ 设备零接触市电 → 无安规认证负担
- ✅ 形态紧凑 → 80×60×25 mm(仅前端板 + LDSDR + 外壳)
- ✅ 候选人 BYO 资产对项目"启动即用",非常少需要 RA 自掏腰包学习新平台
- ✅ 千兆 Ethernet 带宽溢出 → 双通道 IQ 并行采集成为可能(论文之外的能力)
